...
def plot_generated_images(epoch, generator, org_img, examples=100, dim=(10, 10), figsize=(10, 10)):
    # noise = np.random.normal(0, 1, size=[examples, random_dim])
    generated_images = generator.predict(org_img)
    generated_images = generated_images.reshape(128, 28, 28)
...
def train(epochs=1, batch_size=128):
    ...
    for e in range(1, epochs+1):
        ...
        if e == 1 or e % 10 == 0:
            image_batch = x_train[np.random.randint(0, x_train.shape[0], size=batch_size)]
            plot_generated_images(e, generator, image_batch)
